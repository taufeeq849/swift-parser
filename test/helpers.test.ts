...
describe("Helpers", () => {
  describe("Bank date parser", () => {
    it("should work at all (with full date)", () => {
      expect("2016-12-01").toEqual(
        helpers.Date.parse(2016, 12, 1).toISOString().substr(0, 10)
      );
      expect("1996-12-01").toEqual(
        helpers.Date.parse(1996, 12, 1).toISOString().substr(0, 10)
      );
    });

    it("should work with YY (2-digit) year", () => {
      expect("2016-12-01").toEqual(
        helpers.Date.parse(16, 12, 1).toISOString().substr(0, 10)
      );
    });
  });
  ...
  describe("Bank amount", () => {
    describe("Parse", () => {
      ...
      it("should parse credit amount reversal -> negative", () => {
        expect(helpers.Amount.parse("RC", "123.34")).toEqual(
          BigNumber(-123.34)
        );
      });
      ...
      it("should fail if wrong indicator passed", () => {
        expect(helpers.Amount.parse.bind(null, "X", "123,34")).toThrow(
          /Wrong debit/
        );
      });

      it("should fail if wrong indicator passed / reversal", () => {
        expect(helpers.Amount.parse.bind(null, "XZ", "123,34")).toThrow(
          /reversal\/expected/
        );
      });

      it("should fail if wrong indicator passed / reversal 2", () => {
        expect(helpers.Amount.parse.bind(null, "RZ", "123,34")).toThrow(
          /Wrong debit/
        );
      });

      it("should fail if wrong amount passed", () => {
        expect(helpers.Amount.parse.bind(null, "D", "XXXXXX")).toThrow(
          /Amount cannot be parsed/
        );
      });

      it("should fail for negative amount string", () => {
        expect(helpers.Amount.parse.bind(null, "D", "-123.78")).toThrow(
          /Positive amount/
        );
      });
    });
