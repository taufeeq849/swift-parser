...
describe("Parser", () => {
  ...
  describe("Parser methods", () => {
    ...
    it("_parseLines", () => {
      ...
      expect(result[5]).toEqual({
        id: "61",
        subId: "",
        data: ["1405070507C500,00NTRFNONREF//AUXREF"],
      });
    ...
    it("_groupTags", () => {
      ...
      const result = parser._groupTags([
        ...DUMMY_GROUP_SIMPLE,
        ...DUMMY_GROUP_COMPLEX,
      ]);
  ...
  describe("Integration test", () => {
    ...
    it("statement with structured 86", () => {
      ...
      let result = parser.parse({
        data: DUMMY_STATEMENT_LINES_WITH_STRUCTURE.join("\n"),
        type: "mt940",
      });
      expect(result.length).toEqual(1);
    ...
    it("statement with fields 64, 65, long 61 and statement comment", () => {
      ...
      const result = parser.parse({
        data: DUMMY_STATEMENT_LINES_61_64_65.join("\n"),
        type: "mt940",
      });
      expect(result.length).toEqual(1);
      expect(result[0]).toEqual(exp);
    ...
    it("multiple statements with message blocks", () => {
      ...
      const result = parser.parse({
        data: DUMMY_STATEMENT_W_MESSAGE_BLOCKS.join("\n"),
        type: "mt940",
      });
  ...
  describe("NS tests", () => {
    ...
    it("statement with NS", () => {
      const parser = new Parser();
      const result = parser.parse({
        data: DUMMY_STATEMENT_LINES_WITH_NS.join("\n"),
        type: "mt940",
      });
      expect(result.length).toEqual(1);
      expect(result[0]).toEqual({
